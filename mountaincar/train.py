...
from model_temporal import QNet
...
state_size= env.observation_space.shape[0]*20
...
def CodingState(s) :
    ...
    for i in range(2):
        for j in range(20):
            cs[j+i*20] -= min(stats.norm.pdf(j-9.7, s[i], 1.2) * 15, 5.)
...
class DQNAgent:
    ...
    def replay(self, batch_size):
        ...
        #temporalLayerWeights = [x.weight for x in self.model.children() if isinstance(x, TemporalLayer)]
...
for e in range(n_episodes):
    ...
    for time in range(7000):
        ...
        if done:
            ...
            print('episode: {}/{}, score: {}, e {:.2}, help: {}, reward: {}, 100score avg: {}'.format(e, n_episodes, time, agent.epsilon, state, reward, scores_avg), flush=True)
